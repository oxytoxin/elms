<div class="relative flex flex-col min-h-screen">
    <div class="relative z-20 flex flex-col flex-1">
        ...
        <div class="flex flex-col flex-1 p-3 font-semibold bg-opacity-75 links bg-primary-600 min-h-halfscreen">
            ...
            <div class="actions">
                <a href="{{ route('student.tasks',['task_type'=>1]) }}" class="flex {{ strpos(url()->current(),'tasks/1') ? 'bg-gray-400' : '' }} items-center p-2 bg-opacity-75 rounded-md hover:bg-gray-400 item">
                    <i class="mr-2 icofont-ui-folder"></i>
                    <h1 class="text-sm font-semibold">Assigments</h1>
                </a>
                <a href="{{ route('student.tasks',['task_type'=>2]) }}" class="flex {{ strpos(url()->current(),'tasks/2') ? 'bg-gray-400' : '' }} items-center p-2 bg-opacity-75 rounded-md hover:bg-gray-400 item">
                    <i class="mr-2 icofont-ui-folder"></i>
                    <h1 class="text-sm font-semibold">Quizzes</h1>
                </a>
                <a href="{{ route('student.tasks',['task_type'=>3]) }}" class="flex {{ strpos(url()->current(),'tasks/3') ? 'bg-gray-400' : '' }} items-center p-2 bg-opacity-75 rounded-md hover:bg-gray-400 item">
                    <i class="mr-2 icofont-ui-folder"></i>
                    <h1 class="text-sm font-semibold">Activities</h1>
                </a>
                <a href="{{ route('student.tasks',['task_type'=>4]) }}" class="flex {{ strpos(url()->current(),'tasks/4') ? 'bg-gray-400' : '' }} items-center p-2 bg-opacity-75 rounded-md hover:bg-gray-400 item">
                    <i class="mr-2 icofont-ui-folder"></i>
