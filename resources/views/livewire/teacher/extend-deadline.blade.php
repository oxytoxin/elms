<div>
    <h1 class="mb-5 text-2xl font-semibold">Extend Task Deadline</h1>
    <div>
    ...
    <div class="mt-5">
        ...
        <button wire:click.prevent="extendDeadline" class="p-3 text-xs font-semibold text-white uppercase rounded-lg hover:bg-primary-600 bg-primary-500">EXTEND
            DEADLINE</button>
    </div>
    @error('days')
        <h1 class="text-sm font-semibold text-red-600">{{ $message }}</h1>
    @enderror
    <div class="my-3">
        <h1 class="font-semibold">SCOPE</h1>
        <div class="flex items-center space-x-2">
            <input wire:model="scope" value="all" type="radio" name="scope" class="cursor-pointer form-radio" id="scope1"><label class="cursor-pointer"
                for="scope1">All students in this section</label>
        </div>
        <div class="flex items-center space-x-2">
            <input wire:model="scope" value="selected" type="radio" name="scope" class="cursor-pointer form-radio" id="scope2"><label class="cursor-pointer"
                for="scope2">Selected students</label>
        </div>
    </div>
    ...
    @if ($scope == 'selected')
        <div class="relative">
            <div class="flex space-x-2">
                <input wire:keydown.enter="addStudent" wire:model="email" autofocus autocomplete="off" type="text" name="student_email" id="student_email"
                    class="flex-grow form-input" placeholder="firstname.lastname@sksu.edu.ph">
                <button wire:click.prevent="addStudent" class="p-3 text-xs font-bold text-white rounded-lg bg-primary-500 hover:bg-primary-600">ADD
                    STUDENT</button>
            </div>
            <div x-show="showQuery" class="absolute top-0 w-full overflow-y-auto bg-white divide-y-2 max-h-48 mt-14">
                @foreach ($students as $stud)
                    <div wire:click="setEmail('{{ $stud->email }}')" wire:key="student-{{ $stud->id }}"
                        class="flex flex-col justify-between px-4 py-2 cursor-pointer md:flex-row">
                        <h1>{{ $stud->name }}</h1>
                        <h1>{{ $stud->email }}</h1>
                    </div>
                @endforeach
