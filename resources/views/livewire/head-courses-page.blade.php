<div class="w-full" x-data="{ showEditCourse:@entangle('showEditCourse')}">
    ...
    <form x-cloak x-show.transition.opacity="showEditCourse" id="course_edit" action="#" class="flex flex-col prounded-lg">
        ...
        <h1 class="text-xs italic font-semibold text-red-600">{{ $message }}</h1>
        @enderror
        <button type="submit" onclick="confirm('Confirm changes?') || event.stopImmediatePropagation()" wire:click.prevent="editCourse" class="px-10 py-2 mt-2 rounded-lg bg-primary-500">Save</button>
        <button wire:click.prevent="" @click="showEditCourse = !showEditCourse" class="px-10 py-2 mt-2 rounded-lg bg-primary-500">Cancel</button>
    </form>
    <h1 class="my-3 text-xl font-bold text-center">COURSE MANAGER</h1>
    ...
    <table class="table min-w-full border-2 border-collapse border-gray-200 divide-y shadow">
        ...
        <tbody class="text-center">
            ...
            <tr class="divide-x">
                ...
                <td>{{ $section->code }}<i onclick="confirm('Confirm removal of faculty member?') || event.stopImmediatePropagation()" wire:click.prevent="removeSection({{ $section->id }},{{ $section->teacher_id }})" class="ml-5 text-red-600 cursor-pointer icofont-trash"></i></td>
            </tr>
            @empty
            <tr>
