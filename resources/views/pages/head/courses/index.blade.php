    ...
    <div class="grid gap-2 mt-5 grid-cols-[repeat(auto-fill,minmax(220px,1fr))]">
        ...
        <div class="w-full overflow-hidden border-4 border-primary-600 h-96">
            ...
            <div class="p-2 text-white h-2/6 bg-secondary-500">
                <h1 class="text-xs text-center">{{ $course->name }}</h1>
                <h1 class="text-sm font-semibold text-center text-orange-500">{{ $course->code }}</h1>
                <h1 class="text-sm font-semibold text-center">sections: {{ $course->sections->count() }}</h1>
            </div>
            <div class="h-1/6">
