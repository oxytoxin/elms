...
<div class="m-4 px-7">
    ...
    @forelse ($tasks as $task)
            <a href="{{ route('student.task',['task'=> $task->id]) }}">
                <div class="relative flex flex-col h-full p-3 text-center {{ $task->deadline && now() > $task->deadline ? 'bg-red-400' : 'bg-white' }} rounded-lg shadow-lg hover:bg-green-300 focus:outline-none min-h-24">
                    <div class="flex-grow">
                        <h1>{{ $task->name }}</h1>
                    ...
                    <h1 class="absolute text-sm font-semibold text-red-600 bottom-2">Date due: {{ $task->deadline ? $task->deadline->format('h:i a, m/d/Y') : 'No deadline set' }}</h1>
                </div>
            </a>
            @empty
